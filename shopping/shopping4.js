...
function discount(x) {
  let dis = shoppingCartItems.map((discountPrice) => {
    return {
      ...
      totalPrice: (discountPrice.quantity * discountPrice.price * 90) / 100,
    };
  });
  return dis;
}
...
function findAverafe() {
  let average = 0;
  let disc = discount(10);
  for (let i = 0; i < disc.length; i++) {
    average = average + disc[i].totalPrice;
  }

  average = average / disc.length;
  console.log(average);
  return average;
}
findAverafe();
...
let avg = discount(10);
let find = findAverafe();
let negzuilk = avg.filter((a) => {
  return a.price > find;
});

console.log(negzuilk);
